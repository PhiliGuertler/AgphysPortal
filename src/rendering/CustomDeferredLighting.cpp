...
namespace Saiga
{
CustomDeferredLighting::CustomDeferredLighting(GBuffer& framebuffer) : gbuffer(framebuffer)
{
    createLightMeshes();
    shadowCameraBuffer.createGLBuffer(nullptr, sizeof(CameraDataGLSL), GL_DYNAMIC_DRAW);
}

CustomDeferredLighting::~CustomDeferredLighting() {}

void CustomDeferredLighting::loadShaders()
{
...
void CustomDeferredLighting::init(int _width, int _height, bool _useTimers)
{
    this->width  = _width;
    this->height = _height;
...
void CustomDeferredLighting::resize(int _width, int _height)
{
    this->width  = _width;
    this->height = _height;
...
void CustomDeferredLighting::cullLights(Camera* cam)
{
...
void CustomDeferredLighting::printTimings()
{
    if (!useTimers) return;
    for (int i = 0; i < 5; ++i)
    {
...
void CustomDeferredLighting::initRender()
{
    totalLights       = 0;
    renderedDepthmaps = 0;
...
void CustomDeferredLighting::renderDepthMaps(CustomDeferredRenderingInterface* renderer)
{
...
void CustomDeferredLighting::render(Camera* cam, const ViewPort& viewPort)
{
...
void CustomDeferredLighting::postprocessVolumetric()
{
    // lazy load
...
void CustomDeferredLighting::setupStencilPass()
{
...
void CustomDeferredLighting::setupLightPass(bool isVolumetric)
{
...
void CustomDeferredLighting::renderDirectionalLights(Camera* cam, const ViewPort& vp, bool shadow)
{
...
void CustomDeferredLighting::renderDebug(Camera* cam)
{
...
void CustomDeferredLighting::blitGbufferDepthToAccumulationBuffer()
{
    //    glEnable(GL_DEPTH_TEST);
...
void CustomDeferredLighting::applyVolumetricLightBuffer()
{
...
void CustomDeferredLighting::setShader(std::shared_ptr<SpotLightShader> spotLightShader,
...
void CustomDeferredLighting::setShader(std::shared_ptr<PointLightShader> pointLightShader,
...
void CustomDeferredLighting::setShader(std::shared_ptr<DirectionalLightShader> directionalLightShader,
...
void CustomDeferredLighting::setShader(std::shared_ptr<BoxLightShader> boxLightShader,
...
void CustomDeferredLighting::setDebugShader(std::shared_ptr<MVPColorShader> shader)
{
    this->debugShader = shader;
}

void CustomDeferredLighting::setStencilShader(std::shared_ptr<MVPShader> stencilShader)
{
...
void CustomDeferredLighting::createLightMeshes()
{
...
void CustomDeferredLighting::initDirectionalLightShader() {
...
std::shared_ptr<DirectionalLight> CustomDeferredLighting::createDirectionalLight() {
...
void CustomDeferredLighting::addDirectionalLight(std::shared_ptr<DirectionalLight> light) {
    initDirectionalLightShader();

	auto findLight = std::find(directionalLights.begin(), directionalLights.end(), light);
	if(findLight == directionalLights.end()) {
		directionalLights.push_back(light);
	}
}

std::shared_ptr<PointLight> CustomDeferredLighting::createPointLight()
{
    if (!pointLightShader)
    {
        const DeferredLightingShaderNames& names = DeferredLightingShaderNames();
...
std::shared_ptr<SpotLight> CustomDeferredLighting::createSpotLight()
{
    if (!spotLightShader)
    {
        const DeferredLightingShaderNames& names = DeferredLightingShaderNames();
...
std::shared_ptr<BoxLight> CustomDeferredLighting::createBoxLight()
{
    if (!boxLightShader)
    {
        const DeferredLightingShaderNames& names = DeferredLightingShaderNames();
...
void CustomDeferredLighting::removeLight(std::shared_ptr<DirectionalLight> l)
{
    directionalLights.erase(std::find(directionalLights.begin(), directionalLights.end(), l));
}

void CustomDeferredLighting::removeLight(std::shared_ptr<PointLight> l)
{
    pointLights.erase(std::find(pointLights.begin(), pointLights.end(), l));
}

void CustomDeferredLighting::removeLight(std::shared_ptr<SpotLight> l)
{
    spotLights.erase(std::find(spotLights.begin(), spotLights.end(), l));
}

void CustomDeferredLighting::removeLight(std::shared_ptr<BoxLight> l)
{
...
void CustomDeferredLighting::renderImGui(bool* p_open)
{
    int w = 340;
    int h = 240;
    ImGui::SetNextWindowPos(ImVec2(680, height - h), ImGuiCond_FirstUseEver);
    ImGui::SetNextWindowSize(ImVec2(w, h), ImGuiCond_FirstUseEver);
    ImGui::Begin("CustomDeferredLighting", p_open);
