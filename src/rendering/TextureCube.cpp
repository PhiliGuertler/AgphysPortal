...
namespace Saiga
{
void TextureCube2::uploadData(GLenum target, const void* data)
{
    //    bind(0);
    bind();
    glTexImage2D(target,
...
void TextureCube2::uploadData(const void* data)
{
    bind();
    for (int i = 0; i < 6; ++i)
    {
...
void TextureCube2::setDefaultParameters()
{
...
bool TextureCube2::fromImage(std::vector<Image>& images)
{
...
void TextureCube2::create(int width, int height, GLenum color_type, GLenum internal_format, GLenum data_type,
                         const void* data)
{
